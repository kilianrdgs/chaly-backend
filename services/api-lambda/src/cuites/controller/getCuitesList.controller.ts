 ...
 *           type: string
 *           enum: [scroll, profile]
 *         description: Contexte 'scroll' pour le feed global, 'profile' pour les cuites d'un utilisateur
 *       - in: query
 *         name: pseudo
 *         required: false
 *         schema:
 ...
 *         schema:
 *           type: string
 *         description: Id de la cuite pour récupérer les cuites associées (alors cursor sera ignoré)
 *     responses:
 *       200:
...
export function getCuitesListController(deps: ServiceDeps): RequestHandler {
	return async (req, res) => {
		...
		const cuiteIdStr = (req.query.cuiteId as string) ?? null;
		...
		try {
			const result = await getCuitesListService(
				...
				userId,
				pseudo,
				cuiteIdStr ? Number.parseInt(cuiteIdStr, 10) : null,
			);
